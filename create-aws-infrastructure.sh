...
master_instance_id=$(aws ec2 run-instances --image-id ami-12345678 --instance-type t2.small --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=master-node-01},{Key=project,Value=wecloud}]' --subnet-id $subnet_id --security-group-ids $security_group_id --region us-east-1 --user-data '#!/bin/bash
apt-get update
...
worker1_instance_id=$(aws ec2 run-instances --image-id ami-12345678 --instance-type t2.micro --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=worker-node-01},{Key=project,Value=wecloud}]' --subnet-id $subnet_id --security-group-ids $security_group_id --region us-east-1 --user-data '#!/bin/bash
apt-get update
apt-get install -y python3.10
apt-get install -y nodejs=18.0
apt-get install -y openjdk-11-jdk
...
worker2_instance_id=$(aws ec2 run-instances --image-id ami-12345678 --instance-type t2.micro --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=worker-node-02},{Key=project,Value=wecloud}]' --subnet-id $subnet_id --security-group-ids $security_group_id --region us-east-1 --user-data '#!/bin/bash
apt-get update
apt-get install -y python3.10
apt-get install -y nodejs=18.0
apt-get install -y openjdk-11-jdk
